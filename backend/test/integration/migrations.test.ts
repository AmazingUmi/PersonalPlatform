...
import { appMigrationTargets, coreMigrationTarget } from "../../src/core/database/startup-migrations.js";
...
describe("app migration scopes", () => {
  ...
  it("only includes enabled apps as targets", async () => {
    const appsDir = join(tmpRoot, "apps");
    ...
    const targets = await appMigrationTargets(tmpRoot, appsDir, db, {});
    const scopes = targets.map((target) => target.scope);
    assert.ok(scopes.includes("enabled_app"));
    assert.ok(!scopes.includes("disabled_app"));
  });
});
